...
```{r cont mod bound, include=TRUE,fig.width=5,fig.height=4}
...
```{r cont mod bound boot, include=TRUE,fig.width=5,fig.height=4}
...
Plotting and other effect estimation tools are not yet enabled for non-linear fits and will produce an error, but we can look at a few aspects of the fit. Here you can see that the regression line is expected to be steeper at higher levels of the modifier, as in previous fits. We can approximate the effect (point estimate only) at a specific value of z by plotting a smooth fit limited a narrow range of the modifier (< -1 or > 1). Here we see that the joint effect of exposures does appear to differ across values of the modifier, but there is little suggestion of non-linearity at either low or high values of the modifier. This informal assessment agrees with intuition based on the estimated coefficients.
```{r cont mod nl plot, include=TRUE, fig.width=6,fig.height=4}
library(ggplot2)
...
ggplot() + 
         ...
         geom_smooth(aes(x=q, y=ey), se=FALSE, color="black", data=plotdata[plotdata$modifier>1,]) + 
         geom_smooth(aes(x=q, y=ey), se=FALSE, color="red", data=plotdata[plotdata$modifier < -1,]) + 
  theme_classic() + 
