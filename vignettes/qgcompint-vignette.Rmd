...
ggplot() + pp0ee + pp1ee + scale_color_brewer(name="")
```
...
#### Overlay plots of predictions at multiple modifier levels<a name="subsec-esteq-overlay"></a>
Visually, it's easier to compare two plots with the same y-axis. Here we can see that the joint regression curve is steeper at M=1 than it is at M=0
...
library(ggplot2) # need to explicitly call ggplot
pp0ee = plot(qfit1ee, emmval=0, suppressprint=TRUE, geom_only=TRUE)
pp1ee = plot(qfit1ee, emmval=1, suppressprint=TRUE, geom_only=TRUE)
# relabel the plots uisn
pp0ee$point$data$col = "M=0"
pp1ee$point$data$col = "M=1"
pp0ee$errorbar$data$col = "M=0"
...
Visually, it's easier to compare predictions across all levels of the EMM variable  on the same plot.
...
# relabel the plots uisn
pp0ee$point$data$col = "Z=0"
pp1ee$point$data$col = "Z=1"
pp2ee$point$data$col = "Z=2"
pp0ee$errorbar$data$col = "Z=0"
pp1ee$errorbar$data$col = "Z=1"
pp2ee$errorbar$data$col = "Z=2"
# jitter along the x axis so the points do not overlap
pp0ee$point$data$x = pp0ee$point$data$x - 0.01 
pp2ee$point$data$x = pp2ee$point$data$x + 0.01 
pp0ee$errorbar$data$x = pp0ee$errorbar$data$x - 0.01 
pp2ee$errorbar$data$x = pp2ee$errorbar$data$x + 0.01 

ggplot() + pp0ee + pp1ee + pp2ee + scale_color_brewer(name="")
```
